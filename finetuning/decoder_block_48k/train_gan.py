...
def parse_args():
    ...
    # Data
    parser.add_argument(
        "--train_shards", type=str, required=True,
        help="WebDataset shard pattern for training data",
    )
    parser.add_argument(
        "--val_shards", type=str, default=None,
        help="WebDataset shard pattern for validation data",
    )

    # Model
    parser.add_argument(
        "--decoder_model_path", type=str, default="Qwen/Qwen3-TTS-Tokenizer-12Hz",
    ...
    parser.add_argument(
        "--resume_generator_from", type=str, default=None,
    ...
    parser.add_argument(
        "--gan_crop_seconds", type=float, default=2.0,
    ...
    # Output
    parser.add_argument("--output_dir", type=str, default="output/decoder_block_48k_gan", help="Output directory")
    parser.add_argument("--save_every", type=int, default=5000, help="Checkpoint save interval (steps)")
    parser.add_argument("--eval_every", type=int, default=1000, help="Evaluation interval (steps)")
    ...
    parser.add_argument("--wandb_project", type=str, default="qwen3-tts-decoder-block-48k", help="WandB project")
...
def build_gan_crops(
    ...
    for i in range(pred.shape[0]):
        ...
        if max_start > 0:
            start = int(torch.randint(0, max_start + 1, (1,), device=pred.device).item())
        else:
            start = 0
        end = start + crop_len
    ...
    padding_ratio = 1.0 - (lengths.float().sum().item() / float(lengths.numel() * max_len))
...
def create_model(args, accelerator):
    ...
    if args.resume_generator_from:
        ...
        if checkpoint_path.exists():
            ...
            accelerator.print(f"WARNING: Generator checkpoint not found at {checkpoint_path}")
...
def main():
    ...
    gan_crop_samples = int(args.gan_crop_seconds * target_sample_rate) if args.gan_crop_seconds > 0 else 0
    ...
    if args.log_with and accelerator.is_main_process:
        ...
        if args.log_with == "wandb":
            ...
            accelerator.init_trackers(project_name=args.wandb_project, config=tracker_config)
    elif args.log_with:
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            with accelerator.accumulate(model, mpd, msd):
                ...
                loss_d_mpd, loss_d_mpd_r, loss_d_mpd_g, dr_mpd, dg_mpd = discriminator_loss(mpd_real_out, mpd_fake_out)

                # MSD
                msd_real_out, _ = msd(target_wav)
                ...
                loss_g_adv_mpd = generator_adversarial_loss(mpd_fake_out_g)
                loss_fm_mpd = feature_matching_loss(mpd_real_fmap, mpd_fake_fmap)

                # MSD (with gradients through generator)
                msd_fake_out_g, msd_fake_fmap = msd(pred_wav)
                msd_real_out_g, msd_real_fmap = msd(target_wav)
                loss_g_adv_msd = generator_adversarial_loss(msd_fake_out_g)
                loss_fm_msd = feature_matching_loss(msd_real_fmap, msd_fake_fmap)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.log_every == 0:
                    log_dict = {
                        ...
                        "gan/crop_len_seconds": gan_crop_len_samples / target_sample_rate,
